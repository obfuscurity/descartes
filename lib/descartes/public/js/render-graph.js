...
var renderGraphs = function() {
  ...
  return $.ajax({
    ...
    for (var i=0; i < d.length; i++) {
      ...
      $('div.graphs div.row span#' + d[i].uuid).append('<a href="/graphs/' + d[i].uuid + '"><img src="' + encodeURI(myGraphUrl) + '" alt="' + d[i].name + '" name="' + d[i].uuid + '" /></a>');
    }
...
$('.tools.btn-group a.tools.btn.trash').on('click', function() {
  if ($('fieldset.in.collapse').length === 0) {
    ...
    $('#destroy_warning ul li button.destroy_submit').on('click', function() {
      ...
      deleteGraphCounter = markedGraphs.length;
      for (var g=0; g < markedGraphs.length; g++) {
        deleteGraph(markedGraphs[g].name, function() {
          deleteGraphCounter -= 1;
          console.log(deleteGraphCounter + ' graphs remaining');
          if (deleteGraphCounter === 0) {
...
// Show "destroy mode" form when enabled
$('.tools.btn-group').on('click')
