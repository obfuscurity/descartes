...
var renderGraphs = function() {
  ...
  return $.ajax({
    ...
    for (var i=0; i < d.graphs.length; i++) {
      ...
      $($($($('div.graphs div.row')[row]) + 'div.graph.span' + spanSize)[i]).append('<label for="' + d.graphs[i].uuid + '">' + d.graphs[i].name + '</label>');
      $($($($('div.graphs div.row')[row]) + 'div.graph.span' + spanSize)[i]).append('<img src="' + encodeURI(myGraphUrl) + '" alt="' + d.graphs[i].name + '" name="' + d.graphs[i].uuid + '" />');
    }
