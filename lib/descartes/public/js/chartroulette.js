...
var renderChatrouletteTitle = function(name) {
  $('.header h1').find('span').remove();
  $('.header h1').append('<span class="filter"> - ' + name + '</span>');
}
...
var renderGraphs = function() {
  $.ajax({
    ...
    if (d.length > 0) {
      ...
      $('div.graphs').append("<p>Sorry, we don't have anything to show you yet. Why don't you favorite (star) a dashboard and come back again?");
...
var renderNextDashboard = function() {
  return $.ajax({
    ...
    renderChatrouletteTitle(d.dashboard.name);
    myDashboardIndex++;
