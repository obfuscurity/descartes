...
var myAreaMode = 'off';
var myLogMode = 'off';
var myNullAsZeroMode = false;

// Update our default settings
var updateLocalSettings = function(settings) {
  myLineMode = settings.lineMode;
  myAreaMode = settings.areaMode;
  myLogMode = settings.logBase;
  myNullAsZeroMode = settings.drawNullAsZero;
};

// Select our active lineMode choice
var selectLineModeButton = function() {
  var index = $($('.line_mode.btn-group button.line_mode.btn:contains(' + myLineMode + ')')).attr('name');
...
var selectAreaModeButton = function() {
  var index = $($('.area_mode.btn-group button.area_mode.btn:contains(' + myAreaMode + ')')).attr('name');
...
var selectLogModeButton = function() {
  var index = $($('.log_mode.btn-group button.log_mode.btn:contains(' + myLogMode + ')')).attr('name');
  ...
  return $.ajax({
    ...
    data: {
      'overrides': {
        ...
        'areaMode': myAreaMode,
        'logBase': myLogMode,
        'drawNullAsZero': myNullAsZeroMode
      }
    },
    dataType: 'json',
...
// Toggle drawNullAsZero option
$('.toggle.null_as_zero').on('click', function() {
  if ($(this).attr('class') === 'toggle null_as_zero success') {
    console.log("disabled");
    myNullAsZeroMode = false;
  } else {
    console.log("enabled");
    myNullAsZeroMode = true;
  }
  updateGraph(function() {
    clearGraphs();
    renderGraphs();
...
// Toggle switch for "Null as Zero" graph options
$('.null_as_zero').toggle({
  text: {
    enabled: 'ENABLED',
    disabled: 'DISABLED'
