...
var renderGraphs = function() {
  ...
  return $.ajax({
    ...
    $('div.graph div.preview').append('<img alt="' + d.name + '" name="' + d.uuid + '" />');
    $.ajax({ 
      beforeSend: function(xhr) {
        var creds = graphiteUser + ':' + graphitePass;
        if (creds.length > 1) {
      ...
      cache: false,
      processData: false,
      url : constructGraphUrl(profileConfig) + '&debug=true'
    }).done(function(b64data){
      console.log(b64data);
      $('div.graph div.preview img').attr('src', 'data:image/png;base64,' + Crypto.util.bytesToBase64(b64data));
    });
