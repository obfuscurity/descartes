...
$('div.description').on('click', 'span.closed#description', function() {
  ...
  $(this).html('<textarea class="description span5" rows="5"></textarea>');
...
var resetDescriptionForm = function(description) {
  var newDescription = (typeof description === 'string') ? description : 'Click here to add your own description of this graph.';
  ...
  if (typeof description === 'string') {
    $('div.description span#description').text(newDescription);
  } else {
    $('div.description span#description').append('<span id="changeme">' + newDescription + '</span>');
  }
...
// Submit description change on enter
$('div.description').on('click', 'a.description_submit', function() {
  ...
  if (newDescription.length > 0) {
    updateObjectAttributes({ 'description': newDescription });
