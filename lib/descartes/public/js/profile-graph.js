...
var renderComposerModal = function() {
  ...
  $.ajax({
    ...
    $('#composer_modal img#preview').attr('src', composerImageUrl);
...
var renderGraphs = function() {
  ...
  return $.ajax({
    ...
    var mergedConfig = $.extend($.parseJSON(d.configuration), $.parseJSON(d.overrides));
    var profileConfig = $.extend($.parseJSON(d.configuration), $.parseJSON(d.overrides), {height: 300, options: '&hideLegend=false', targets: targets});
    var imagePreview = {height: 150, width: $('#composer_modal img#preview').width(), options: '&hideLegend=true', targets: targets};
    var imageConfig = $.extend($.parseJSON(d.configuration), $.parseJSON(d.overrides), imagePreview);

    $('#composer_modal img#preview').attr('src', constructGraphUrl(imageConfig));
    $('#composer_modal textarea').text(JSON.stringify(mergedConfig.target, null, "  "));
...
jwerty.key('alt+cmd+enter/alt+ctrl+c', function() {
