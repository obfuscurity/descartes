...
var renderSparklines = function() {
  for (var target = ((myPageIndex - 1) * myMetricsPerPage); target < (myPageIndex * myMetricsPerPage); target += 1) {
    ...
    if (myLoadedMetrics[target] !== undefined) {
      hash = CryptoJS.SHA256(myLoadedMetrics[target]).toString(CryptoJS.enc.Hex);
      $('div.metrics').append('<div class="default" id="' + hash + '"></div>');
      $.ajax({
        accepts: {jason: 'application/json'},
        cache: false,
        hash: hash,
        target: myLoadedMetrics[target],
        dataType: 'jsonp',
        jsonp: 'jsonp',
        error: function(xhr, textStatus, errorThrown) { console.log(errorThrown); },
        url: graphiteUrl + '/render?target=' + myLoadedMetrics[target] + '&from=-' + myInterval + '&format=json'
        ...
        if (output.length === 1) {
          ...
          graph.render();
          $('.metrics div#' + hash).append('<span class="target"></span>');
