...
var renderDashboards = function(owner) {
  ...
  return $.ajax({
    ...
    for (var i=0; i < d.length; i++) {
      ...
      $($('div.dashboards div.row')[row]).append('<a href="/dashboards/' + d[i].uuid + '"><div class="dashboard span' + spanSize + '"></div></a>');
      $($($($('div.dashboards div.row')[row]) + 'div.dashboard.span' + spanSize)[i]).append('<div class="name" id="' + d[i].uuid + '">' + d[i].name + '</div>');
      $($($($('div.dashboards div.row')[row]) + 'div.dashboard.span' + spanSize)[i]).append('<div class="count">' + d[i].graph_count + '</div>');
      //$($($($('div.dashboards div.row')[row]) + 'div.dashboard.span' + spanSize)[i]).append('<img src="' + encodeURI(myGraphUrl) + '" alt="' + d[i].name + '" name="' + d[i].uuid + '" />');
    }
  });
};
